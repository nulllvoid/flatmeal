...
import type { PollOptionView, TodayPollView } from '@/types/domain';
...
export function useTodayPoll(flatId: string | null | undefined, userId: string | undefined) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: pollRow } = await supabase
      .from('daily_polls')
      .select('id, poll_date, status, winner_recipe_id, winner_reason')
      .eq('flat_id', flatId)
    ...
    const [{ data: optionRows }, { data: voteRows }, { data: memberRows }, { data: attendanceRows }] =
      await Promise.all([
        supabase
          .from('poll_options')
  ...
  return { poll, headcount, castVote, setOutToday, reload: load };
}
